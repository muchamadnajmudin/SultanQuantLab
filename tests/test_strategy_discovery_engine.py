...
def _gap_result(
    ...
    return {
        ...
        "qualified_strategies": (
            qualified if qualified is not None else []
        ),
        "weak_strategies": (
            weak if weak is not None else []
        ),
        "market_context": (
            market_context
...
def test_no_gap_returns_no_gap_status():
    result = discover_strategies(
        _gap_result(gap_detected=False)
    )

    assert result["status"] == DISCOVERY_STATUS_NO_GAP
    assert result["candidates"] == []


def test_none_gap_result_is_safe():
    result = discover_strategies(None)
...
def test_non_dict_gap_result_is_safe():
    result = discover_strategies("invalid")
...
def test_volatile_market_creates_breakout_candidate():
    ...
    assert candidate["method_type"] == "BREAKOUT"
...
def test_unknown_market_creates_hybrid_candidate():
    ...
    assert candidate["method_type"] == "HYBRID"
...
def test_input_is_not_modified():
    ...
    original_context = gap["market_context"].copy()

    discover_strategies(gap)

    assert gap["market_context"] == original_context
...
def test_result_is_independent():
    ...
    result["market_context"]["regime"] = "CHANGED"
    ...
    assert fresh["market_context"]["regime"] == (
        "TREND"
    )
